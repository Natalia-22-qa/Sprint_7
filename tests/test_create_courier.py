...
class TestCreateCourier:
    ...
    def test_create_courier_success(self, registration_courier):
        assert registration_courier[0].status_code == 201 and registration_courier[0].json() == Response.SUCCESS_CREATE_COURIER

    @allure.title('Проверка появления ошибки при создании одинаковых курьеров: POST /api/v1/courier')
    def test_create_courier_clone_error(self, login_courier):
        response = requests.post(f'{Api.URL}{Api.CREATE_COURIER_API}', json=login_courier[1])
        assert response.status_code == 409 and response.json() == Response.CONFLICT_CREATE_COURIER

    @allure.title('Проверка появления ошибки при отсутствии обязательного поля: POST /api/v1/courier')
